...
const Onboardingscreen = ({ navigation }: { navigation: any }) => {
  ...
  const NextCard = async() => {
    ...
    ref.current?.scrollToOffset({ offset: (sliderIndex + 1) * width, animated: true });
  };
  ...
  return (
    <View style={styles.container} >
      ...
      <View style={styles.bottomBox} >
        ...
        <CustomButton title={buttontitle} borderRadius={10} onClick={NextCard} />
      </View>
    </View>
  );
};
...
const Indicator = ({ scrollX }: { scrollX: number }) => {
  return <Animated.View style={styles.dotbox}>
...
const Dot = ({ scrollX, index }: { scrollX: number, index: number }) => {
  ...
  const scale = interpolate(scrollX, inputRange, [0.8, 1.2, 0.8], Extrapolation.CLAMP);
  const widthscale = interpolate(scrollX, inputRange, [0, 10, 0], Extrapolation.CLAMP);
